...
class GeneratedColumnSubscriberTest extends TestCase
{
    ...
    public function testOnGeneratedColumnsBuild(): void
    {
        ...
        Assert::assertCount(9, $generatedColumns);

        $generatedColumns = iterator_to_array($generatedColumns);

        // Skip the first 4 columns for campaign_lead_event_log
        for ($i = 0; $i < 4; ++$i) {
            array_shift($generatedColumns);
        }

        // Test the 5 columns for campaign_leads
