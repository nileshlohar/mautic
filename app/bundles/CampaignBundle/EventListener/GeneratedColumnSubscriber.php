...
class GeneratedColumnSubscriber implements EventSubscriberInterface
{
    ...
    {
        // Keep existing generated columns for campaign_lead_event_log
        $event->addGeneratedColumn($this->buildEventLogColumn('hour', 'DATETIME', '%Y-%m-%d %H:00:00'));
        $event->addGeneratedColumn($this->buildEventLogColumn('day', 'DATE', '%Y-%m-%d'));
        $event->addGeneratedColumn($this->buildEventLogColumn('month', 'DATE', '%Y-%m-01'));
        $event->addGeneratedColumn($this->buildEventLogColumn('year', 'DATE', '%Y-01-01'));

        // Add new generated columns for campaign_leads
    ...
    private function buildEventLogColumn(string $unit, string $type, string $format): GeneratedColumn
    {
